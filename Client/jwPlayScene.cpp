...
namespace jw
{
	...
	void PlayScene::Initialize()
	{
		...
		mCupheadAnimator->GetCompleteEvent(L"CupheadIntro") = std::bind(&PlayScene::CupheadIntroCompleteEvent, this);
	...
	void PlayScene::Update()
	{
		...
		if (mBossCarrot != nullptr && mBossCarrot->GetIsBossOut() && !CarrotDeadChecker)
		{
			...
			SceneManager::SetNextSceneType(eSceneType::Ending);
	...
	void PlayScene::CupheadIntroCompleteEvent()
	{
		mCupheadAnimator->Play(L"CupheadIdle_R", true);
	}

	void PlayScene::BossIntroCompleteEvent()
	{
		
	}

	void PlayScene::FightTextShow()
	{
		ftw = object::Instantiate<FightText_WALLOP>(Vector2(800.0f, 900.0f), eLayerType::UI);
		mFightTextAnimator = ftw->GetComponent<Animator>();
		mFightTextAnimator->GetCompleteEvent(L"ImageFightText") = std::bind(&PlayScene::FightStart, this);
